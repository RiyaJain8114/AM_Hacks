...
export const ChatInterface = ({ onClose }: ChatInterfaceProps) => {
  ...
  const getResponseForAction = async (action: string, subAction?: string): Promise<string> => {
    try {
      setIsLoading(true);
  ...
  const handleQuickAction = async (action: string, isFollowUp: boolean = false) => {
    if (!isFollowUp) {
      ...
      const response = await getResponseForAction(action);
      
      setMessages(prev => prev.map((msg, idx) => 
        idx === prev.length - 1 
          ? { ...msg, text: response, isLoading: false, showQuickActions: true }
          : msg
      ));
    } else {
      ...
      const response = await getResponseForAction(currentCategory || '', action);
  ...
  const handleSendMessage = async (text: string = inputMessage) => {
    if (text.trim()) {
      ...
      try {
        ...
        // Temporary mock response
        await new Promise(resolve => setTimeout(resolve, 1000));
        const response = "I'll help you with that query. Please provide more details if needed.";
        ...
        setMessages(prev => prev.map((msg, idx) => 
          idx === prev.length - 1 
            ? { ...msg, text: "I apologize, but I'm having trouble processing your request. Please try again later.", isLoading: false, error: "Failed to send message" }
            : msg
        ));
      }
    }
